...
# $author:  Brandon Schwartz \$@ Qualcomm Inc.		                       |
# $date:    08/08/2007                                                         |
...
if ($opts{'reboot'}) {
    ...
    $ssh = _getSSHConn($ssh);
    $ssh->send("user add --domain qcom --id $userObj{'uid'},$userObj{'gid'} --password hurry^10 nas-user $userObj{'user'}\n");
    $ssh->send("user group grant $userObj{'group'} $userObj{'user'}\@qcom\n");
	$ssh->send("quit\n");
	$ssh->expect($timeout,"closed.");
    $ssh->soft_close();
    ...
    ($host) = @machines;
    $ssh = _getSSHConn($ssh);
    $ssh->send("user group revoke $userObj{'group'} $userObj{'user'}\@qcom\n");
    ...
    foreach my $path (@readOnlyPaths) {
	    if ($opts{'grant'}) {
	        $ssh->send_slow(0,"cryptainer grant read $path samsung\@qcom\n");
            	foreach my $ipRange (@allRackRanges) {
	            $ssh->send_slow(0,"cryptainer ip grant read $path $ipRange\n");
            	}
	    } else {
		    $ssh->send_slow(0,"cryptainer revoke access,change-perms $path samsung\@qcom\n");
            	foreach my $ipRange (@allRackRanges) {
    ...
    foreach my $path (@readWritePaths) {
	    if ($opts{'grant'}) {
		    $ssh->send_slow(0,"cryptainer grant access $path samsung\@qcom\n");
            	foreach my $ipRange (@allRackRanges) {
	...
	if ($opts{'grant'}) {
		...
		$ssh->soft_close();
		@listPath = $ssh->before();
		if (!(grep (/samsung/, @listPath))) {
			print "ERROR, Samsung not granted ACLs\n";
		} 
	} else {
		$ssh->send("cryptainer acl list\n");
		$ssh->send("quit\n");
		...
		@listPath = $ssh->before();
		if (grep (/samsung/, @listPath)) {
			print "ERROR, Samsung ACLs not removed.\n";
		}
	}
} elsif($batchFile) {
...
B. Schwartz - <brandons@qualcomm.com> - Qualcomm Inc,.

=head1 COPYRIGHT

Copyright (C) 2007 by Qualcomm Inc,.

This script was written for Qualcomm Inc., and was not intended for external use.  Qualcomm reserves all the rights for this script. Copyright (c) 2007.
